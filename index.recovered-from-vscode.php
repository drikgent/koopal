...
try {
    ...
    if ($page === 1 && empty($searchTerm)) {
        ...
        // Fetch Top 10 Most Visited Series
    $top_series = [];
    try {
        $stmt = $pdo->query("
...
.continue-list {
    padding: 15px;
    overflow-y: auto;
    max-height: calc(30vh - 30px); /* Adjust for header height */
}
                ...
                <?php foreach ($top_series as $index => $series): ?>
                    <li class="rank-item">
                        ...
                        <div class="rank-views">
                            <span>views: <?= number_format($series['views'])?></span>
                        </div>
                    </li>
                    <?php endforeach; ?>
            </ul>
    ...
    <div class="continue-list">
        ...
        <div class="continue-item">
            ...
            <div class="continue-info">
                ...
                <a href="read.php?series_id=<?= $cr['series_id'] ?>&chapter_id=<?= $cr['chapter_id'] ?>">Resume →</a>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
